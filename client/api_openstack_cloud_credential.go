...
func (r ApiOpenstackListRequest) DomainId(domainId int32) ApiOpenstackListRequest {
	r.domainId = &domainId
	return r
}
...
type ApiOpenstackStorageTypesRequest struct {
	ctx                           context.Context
	ApiService                    *OpenstackCloudCredentialAPIService
	openstackStorageTypeListQuery *OpenstackStorageTypeListQuery
}

func (r ApiOpenstackStorageTypesRequest) OpenstackStorageTypeListQuery(openstackStorageTypeListQuery OpenstackStorageTypeListQuery) ApiOpenstackStorageTypesRequest {
	r.openstackStorageTypeListQuery = &openstackStorageTypeListQuery
	return r
}

func (r ApiOpenstackStorageTypesRequest) Execute() ([]string, *http.Response, error) {
	return r.ApiService.OpenstackStorageTypesExecute(r)
}

/*
OpenstackStorageTypes Openstack storage type list

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiOpenstackStorageTypesRequest
*/
func (a *OpenstackCloudCredentialAPIService) OpenstackStorageTypes(ctx context.Context) ApiOpenstackStorageTypesRequest {
	return ApiOpenstackStorageTypesRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
//
//	@return []string
func (a *OpenstackCloudCredentialAPIService) OpenstackStorageTypesExecute(r ApiOpenstackStorageTypesRequest) ([]string, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "OpenstackCloudCredentialAPIService.OpenstackStorageTypes")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/openstack/storage-types"
	...
	// body params
	localVarPostBody = r.openstackStorageTypeListQuery
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["Bearer"]; ok {
