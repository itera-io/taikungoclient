...
type ApiKubernetesLivePodRequest struct {
	ctx context.Context
	ApiService *KubernetesAPIService
	projectId int32
}

func (r ApiKubernetesLivePodRequest) Execute() (*http.Response, error) {
	return r.ApiService.KubernetesLivePodExecute(r)
}

/*
KubernetesLivePod Watch live pods

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param projectId
 @return ApiKubernetesLivePodRequest
*/
func (a *KubernetesAPIService) KubernetesLivePod(ctx context.Context, projectId int32) ApiKubernetesLivePodRequest {
	return ApiKubernetesLivePodRequest{
		ApiService: a,
		ctx: ctx,
		projectId: projectId,
...
// Execute executes the request
func (a *KubernetesAPIService) KubernetesLivePodExecute(r ApiKubernetesLivePodRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "KubernetesAPIService.KubernetesLivePod")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/kubernetes/{projectId}/live-pod"
	localVarPath = strings.Replace(localVarPath, "{"+"projectId"+"}", url.PathEscape(parameterValueToString(r.projectId, "projectId")), -1)
