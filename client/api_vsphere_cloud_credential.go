...
func (r ApiVsphereHypervisorListRequest) Execute() ([]HypervisorSummary, *http.Response, error) {
...
func (a *VsphereCloudCredentialAPIService) VsphereHypervisorListExecute(r ApiVsphereHypervisorListRequest) ([]HypervisorSummary, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  []HypervisorSummary
	)
...
type ApiVsphereValidateVsphereHypervisorsRequest struct {
	ctx context.Context
	ApiService *VsphereCloudCredentialAPIService
	validateVsphereHypervisorsCommand *ValidateVsphereHypervisorsCommand
}

func (r ApiVsphereValidateVsphereHypervisorsRequest) ValidateVsphereHypervisorsCommand(validateVsphereHypervisorsCommand ValidateVsphereHypervisorsCommand) ApiVsphereValidateVsphereHypervisorsRequest {
	r.validateVsphereHypervisorsCommand = &validateVsphereHypervisorsCommand
	return r
}

func (r ApiVsphereValidateVsphereHypervisorsRequest) Execute() (*http.Response, error) {
	return r.ApiService.VsphereValidateVsphereHypervisorsExecute(r)
}

/*
VsphereValidateVsphereHypervisors Validate vSphere hypervisors

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiVsphereValidateVsphereHypervisorsRequest
*/
func (a *VsphereCloudCredentialAPIService) VsphereValidateVsphereHypervisors(ctx context.Context) ApiVsphereValidateVsphereHypervisorsRequest {
	return ApiVsphereValidateVsphereHypervisorsRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
func (a *VsphereCloudCredentialAPIService) VsphereValidateVsphereHypervisorsExecute(r ApiVsphereValidateVsphereHypervisorsRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "VsphereCloudCredentialAPIService.VsphereValidateVsphereHypervisors")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/vsphere/validate/hypervisors"
	...
	// body params
	localVarPostBody = r.validateVsphereHypervisorsCommand
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["Bearer"]; ok {
